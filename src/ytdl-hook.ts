import { setHTTPHeaders, isSafeURL, edlEscape, optionWasSet, getIndexFromYouTubePlaylist, edlTrackJoined } from "./utils";
...
function ytdlSuccess(url: string, json: YTDL.Entity, option: TempOption) {
    ...
    if (json.direct) {
        ...
        const isSelfRedirectingURL = json.entries[0]._type !== "url_transparent" &&
            json.entries[0].webpage_url &&
            json.entries[0].webpage_url === json.webpage_url;

        if (isSelfRedirectingURL) {
            if (json.entries.length > 1 && json.entries[0].protocol === "m3u8_native" && json.entries[0].url) {
