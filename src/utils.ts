...
const safeProtos = new Set(["http", "https", "ftp", "ftps", "rtmp", "rtmps", "rtmpe", "rtmpt", "rtmpts", "rtmpte", "data"]);
...
export function getIndexFromYouTubePlaylist(url: string, json: YTDL.Playlist) {
    ...
    const query = url.substr(index + 1).split("&").map(x => x.split("="));
    ...
    if (maybeIdx && json.entries.length >= maybeIdx &&
...
export function edlTrackJoined(fragments: YTDL.URLLike[], protocol?: YTDL.Protocol, isLive?: boolean, base?: string) {
