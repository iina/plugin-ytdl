import { setHTTPHeaders, isSafeURL, optionWasSet, edlTrackJoined, optionWasSetLocally } from "./utils";
...
function processVideo(reqfmts: YTDL.Video[], json?: YTDL.Entity) {
    ...
    if (opt.use_manifests && isValidManifest(json)) {
        ...
        streamURL = mpdURL;
        if (reqfmts) {
            maxBitrate = Math.max.apply(null, reqfmts.map(fmt => fmt.tbr));
        } else if (json.tbr) {
        ...
        for (const track of reqfmts) {
            const edlTrack = edlTrackJoined(track.fragments, track.protocol,
            ...
            if (track.vcodec && track.vcodec !== "none") {
                ...
                mpv.command("audio-add",
                    [edlTrack || track.url, "auto", track.format_note || ""]);
            }
        }
    } else if (json.url) {
        const edlTrack = edlTrackJoined(json.fragments, json.protocol,
    ...
    if (json.requested_subtitles) {
        Object.keys(json.requested_subtitles).forEach(lang => {
            ...
            const sub = subInfo.data ? `memory://${subInfo.data}` :
                subInfo.url && isSafeURL(subInfo.url) ? subInfo.url : null;
...
export function addVideo(json: YTDL.Entity) {
    ...
    if (json.formats) {
        ...
        for (const f of json.formats) {
            if (f.vcodec === "none") {
                audioItem.addSubMenuItem(menu.item(
                    f.format,
                    () => {
                    ...
                    f.format_id === currentAudioFormat,
                ));
            } else {
                videoItem.addSubMenuItem(menu.item(
                    f.format,
                    () => {
                    ...
                    f.format_id === currentVideoFormat
                ));
            }
        }
